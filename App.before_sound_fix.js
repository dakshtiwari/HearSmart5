 ...
 * - Threshold rule: as soon as 2 responses occur in the most recent 3 valid
 *   ascending trials at a level, that level is accepted immediately — a 3rd
 *   trial is never forced if 2 heard responses already came in on the first
 *   2 attempts.
 *
 * Required packages:
...
const ANDROID_MIN_MASTER_VOLUME = 0.5;
const ANDROID_MAX_MASTER_VOLUME = 1.0;

const MIN_DB = -10;
// A conservative prototype ceiling because generic headphones are uncalibrated.
const MAX_DB = 80;
// Starting level for the very first trial of each test run. 20 dB HL is
// close to normal-hearing threshold for most people, so most users had to
// sit through several quiet, hard-to-hear ascending trials before the tone
// became audible at all — slow and discouraging. 40 dB HL is comfortably
// audible for the vast majority of people (only moderate-or-worse losses
// would miss it), so the test starts fast and still brackets down/up from
...
export default function App() {
  ...
  function handleTestResponse(heard) {
    ...
    if (validAscending) {
      ...
      // BUG FIX: previously gated on `window.length === 3`, so 2 heard
      // responses out of the first 2 ascending trials at this level were
      // recorded but never checked — the app kept demanding a 3rd trial
      // before it would ever advance to the next frequency. 2 heard
      // responses already guarantees "at least 2 of the most recent 3", so
  ...
  function beginTestingCurrentEar() {
    // BUG FIX: this used to compute `practiceDb - 30`, which for the normal
    // practiceDb=30 starting point gave a real test start level of 0 dB HL —
    // nowhere near the intended DEFAULT_START_DB, and inaudibly quiet for a
    // first trial. Now the test always starts at DEFAULT_START_DB (40 dB HL)
    // at minimum, or at the level the practice tone actually required if
    // that ear needed something louder than the default to be heard.
